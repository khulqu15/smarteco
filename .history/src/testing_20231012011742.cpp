...
int main(int argc, char** argv)
{
    ...
    cv::rectangle(resizedImg, cv::Point(0, 0), cv::Point(1200, 400), dominantColor, -1);
    string text = to_string((int)dominantColor[0]) + ", " + to_string((int)dominantColor[1]) + ", " + to_string((int)dominantColor[2]);
    cv::putText(resizedImg, text, cv::Point(50, 220), cv::FONT_HERSHEY_COMPLEX, 0.5, cv::Scalar(255, 255, 255), 2);
